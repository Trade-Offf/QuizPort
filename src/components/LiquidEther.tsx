...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class CommonClass {
      ...
      resize() {
        // 使用文档完整尺寸，确保背景随页面增长而扩展（避免底部露白）
        const doc = document.documentElement;
        const fullW = Math.max(doc.scrollWidth, doc.clientWidth, window.innerWidth);
        const fullH = Math.max(doc.scrollHeight, doc.clientHeight, window.innerHeight);
        this.width = Math.max(1, Math.floor(fullW));
        this.height = Math.max(1, Math.floor(fullH));
...
`;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: any) {
        ...
        // ensure interaction even when canvas is behind other content
        this.winMouseMove = (e: MouseEvent) => Mouse.onDocumentMouseMove(e);
      ...
      dispose() {
        try {
          ...
          if (this.winMouseMove)
            window.removeEventListener('mousemove', this.winMouseMove as any);
          if (this.winTouchStart)
            window.removeEventListener('touchstart', this.winTouchStart as any);
          if (this.winTouchMove)
            window.removeEventListener('touchmove', this.winTouchMove as any);
          if (this.winTouchEnd)
            window.removeEventListener('touchend', this.winTouchEnd as any);
          Mouse.dispose();
          if (Common.renderer) {
